...
(function ($, app, undefined) {
    ...
    var methods = {
		...
		// apply alignment by using getPlugin("contextMenu").executeCommand for correct work of undo/redo actions
        left: function () {
			this.getEditor().getPlugin("contextMenu").executeCommand("alignment:left");
            this.getEditor().render();
        },
        right: function () {
			this.getEditor().getPlugin("contextMenu").executeCommand("alignment:right");
            this.getEditor().render();
        },
        center: function () {
			this.getEditor().getPlugin("contextMenu").executeCommand("alignment:center");
            this.getEditor().render();
        },
        top: function () {
			this.getEditor().getPlugin("contextMenu").executeCommand("alignment:top");
            this.getEditor().render();
        },
        middle: function () {
			this.getEditor().getPlugin("contextMenu").executeCommand("alignment:middle");
            this.getEditor().render();
        },
        bottom: function () {
			this.getEditor().getPlugin("contextMenu").executeCommand("alignment:bottom");
            this.getEditor().render();
        },
        row: function () {
        ...
        link: function () {
            var editor = this.getEditor(),
                ...
                var textForPopup = selectedText ? selectedText : textFromCell;

                $urlDialog.find('.link-text').val(textForPopup);
                $urlDialog.dialog({
                    width: 400,
                    modal: true,
                    buttons: [
                        ...
                        },
                        {
                            ...
                            click: function() {
                                var target = $urlDialog.find('.link-target').is(':checked') ? '_blank' : '_self',
                                	url = $urlDialog.find('.url').val(),
									text = $urlDialog.find('.link-text').val(),
                               		insertToField = '<a href="' + url + '" target="' + target + '">' + textFromCell + '</a>',
									selectedTextReg = selectedText.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&');

                                if(selectedText && (new RegExp(selectedTextReg, 'i')).test(textFromCell)) {
		...
		setColumnWidth: function() {
			if(app.Editor.Hot.getSelectedRange()) {
				$setColumnWidthDialog.dialog('open');
			} else {
		...
		setMultipleColumnsSorting: function() {
			if(app.Editor.Hot.getSelectedRange()) {
				$setMultipleColumnsSortingDialog.dialog('open');
			} else {
				alert('You must select at least one cell in column.');
			}
